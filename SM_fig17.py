...
print (np.round(d[0:4,0],5))
print (np.round(g[0:4,0:4],5))
...
foo_fig.savefig(str(index)+'gd_C_T_Mmax'+str(Mmax)+'W'+str(W)+'alpha'+str(alpha)+'.jpg', format='jpg', dpi=1000,bbox_inches = 'tight')
...
foo_fig = plt.gcf() # 'get current figure'
foo_fig.savefig(str(index)+'gd_direct_beta_Mmax'+str(Mmax)+'W'+str(W)+'.jpg', format='jpg', dpi=1000,bbox_inches = 'tight')
...
foo_fig = plt.gcf() # 'get current figure'
foo_fig.savefig(str(index)+'gd_SPADE_beta_Mmax'+str(Mmax)+'W'+str(W)+'.jpg', format='jpg', dpi=1000,bbox_inches = 'tight')



matrix=d[0:4].T

# Compute the log scale while keeping track of signs
matrix_log_signed = np.sign(matrix) * np.log10(np.abs(matrix))

# Create a grid plot with the signed logarithmic scale
plt.figure(figsize=(6, 6))
plt.imshow(matrix_log_signed, cmap='coolwarm', interpolation='nearest', vmin=-5, vmax=5)
#plt.colorbar()

# Annotate the grid with values in signed log scale
for i in range(matrix.shape[0]):
    for j in range(matrix.shape[1]):
        plt.text(j, i, f"{matrix_log_signed[i, j]:.2f}", ha='center', va='center', color='black', fontsize=10)
...
#plt.title('Signed Logarithmic Grid Plot of Matrix Values')
...
# Save the plot to a local file
plt.savefig("grid_g.png", dpi=300, bbox_inches='tight')
