...
product2,Q2 = inner_product_directSPADE(vectors,nmax,M)
...
product3,Q3 = inner_product(vectors)
...
xmajorLocator   = MultipleLocator(5) #将x轴次刻度标签设置为5的倍数
...
#plt.ylim([1e-4,1e15])
#plt.plot(y, -Q2[0][:,0], linewidth=2, linestyle='-', label='bet1')
plt.fill_between(y, -Q2[0][:,0], y2=0, color='lightblue', alpha=0.5)
plt.fill_between(y, -Q2[1][:,0], y2=0, color=(1.0, 0.8, 0.5), alpha=0.5)
plt.plot(y,Q2[0][:,1],linewidth=2,linestyle='-',label='bet1')
plt.plot(y,Q2[1][:,1],linewidth=2,linestyle='dashed',label='bet1')

#plt.legend(fontsize=17,loc='center left', bbox_to_anchor=(1, 0.5))  

foo_fig = plt.gcf() # 'get current figure'
foo_fig.savefig('separate_POVM_M'+str(M)+'L'+str(L)+'.jpg', format='jpg', dpi=1000,bbox_inches = 'tight')
...
plt.fill_between(y, -Q2[0][:,0], y2=0, color='lightblue', alpha=0.5)
plt.fill_between(y, -Q2[1][:,0], y2=0, color=(1.0, 0.8, 0.5), alpha=0.5)
plt.plot(y,Q3[:,2],linewidth=2,linestyle='-',label='bet1')
plt.plot(y,Q3[:,3],linewidth=2,linestyle='dashed',label='bet1')

#plt.legend(fontsize=17,loc='center left', bbox_to_anchor=(1, 0.5))  

foo_fig = plt.gcf() # 'get current figure'
foo_fig.savefig('orthogonal_POVM_M'+str(M)+'L'+str(L)+'.jpg', format='jpg', dpi=1000,bbox_inches = 'tight')
...
#ymajorLocator = LogLocator(base=10)  # LogLocator for 10^4n
#xmajorFormatter = FormatStrFormatter('%1.1f') #设置x轴标签文本的格式
xmajorLocator   = MultipleLocator(10) #将x轴次刻度标签设置为5的倍数
ymajorLocator   = MultipleLocator(0.05)
cx.xaxis.set_major_locator(xmajorLocator)
#ax.xaxis.set_major_formatter(xmajorFormatter)
cx.yaxis.set_major_locator(ymajorLocator)
