...
if __name__ == '__main__':
    ...
    # — Finally: parallelized classification success‐rate simulations —
    which_alpha = -1
    total_task = 20
    repeats = 20
    sam_nums = [int(1e4), int(1e7), int(1e10)]
    W_test = 1000

    # direct‐imaging
    P_DI = Prob_array_direct_imaging(C1, alpha[which_alpha], x)
    P_sSPADE = Prob_array_separate_SPADE(C2, C2_extra, alpha[which_alpha], x)
    P_oSPADE = Prob_array_orthogonal_SPADE(C3, C3_extra, alpha[which_alpha], x)

    

    # direct imaging
    rates_DI = parallel_success_rates(
        P_DI, r1[:, :, which_alpha],
        total_task, repeats, sam_nums, W_test,
        M, n, L,
        classify_P4,
        C1, C2, C2_extra, C3, C3_extra,
        alpha, 'DI', which_alpha
    )
    
    print(" Direct Imaging rates done")
    # separate SPADE
    rates_sSPADE = parallel_success_rates(
        P_sSPADE, r2[:, :, which_alpha],
        total_task, repeats, sam_nums, W_test,
        M, n, L,
        classify_P4,
        C1, C2, C2_extra, C3, C3_extra,
        alpha, 'sSPADE', which_alpha
    )
    print("Separate SPADE rates done")
    # orthogonal SPADE
    rates_oSPADE = parallel_success_rates(
        P_oSPADE, r3[:, :, which_alpha],
        total_task, repeats, sam_nums, W_test,
        M, n, L,
        classify_P4,
        C1, C2, C2_extra, C3, C3_extra,
        alpha, 'oSPADE', which_alpha
    )
    print(" Orthogonal SPADE rates done")
    #print("DI rates:",   rates_DI)
    #print("sSPADE rates:", rates_sSPADE)
    #print("oSPADE rates:", rates_oSPADE)

    to_save = {
        # the three success‑rate dicts
        'rates_DI':      rates_DI,
        'rates_sSPADE':  rates_sSPADE,
        'rates_oSPADE':  rates_oSPADE,
    ...
    with open('success_plot_data.pkl', 'wb') as f:
        pickle.dump(to_save, f)
    print("Saved all plotting data to success_plot_data.pkl")    
    
    # load and re-plot
    with open('success_plot_data.pkl', 'rb') as f:
        data = pickle.load(f)
