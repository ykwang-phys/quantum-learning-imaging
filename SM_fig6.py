...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul  2 16:17:21 2025
...
def GD_fast_new(N, M, L, sigma):
    l=L/M
    d, g = gd_generation(N, M)
    Mv = int((L +8*sigma)/L * M)
    dis = (L +8*sigma) / Mv
    #print (d[0:4],g[0:4,0:4])
    u_array = np.linspace(-L/2, L/2, M)
    v_array = np.linspace(-L/2 -4*sigma, L/2 +4*sigma, Mv)
...
L=np.linspace(20,80,n).astype(int)
val_array=[]
vec_array=[]
...
for i in range(n):
    M=L[i]*5
    temp_val,temp_vec,CT_array1[i]=spectrum(N,M,L[i],sigma1,S)
    val_array.append(temp_val)
    vec_array.append(temp_vec)
    print (i,time.time()-t0,CT_array1[i]/L[i])
...
for i in range(n):
    M=L[i]*5
    temp_val,temp_vec,CT_array2[i]=spectrum(N,M,L[i],sigma2,S)
    val_array.append(temp_val)
    vec_array.append(temp_vec)
    print (i,time.time()-t0,CT_array2[i]/L[i])
...
for i in range(n):
    M=L[i]*5
    temp_val,temp_vec,CT_array3[i]=spectrum(N,M,L[i],sigma3,S)
    val_array.append(temp_val)
    vec_array.append(temp_vec)
    print (i,time.time()-t0,CT_array3[i]/L[i])
...
#plt.ylim([0,70])
plt.plot(L,CT_array1,linewidth=2,linestyle='-',label='sigma=1')
plt.plot(L,CT_array2,linewidth=2,linestyle='--',label='sigma=2')
plt.plot(L,CT_array3,linewidth=2,linestyle='-.',label='sigma=3')
#plt.legend(fontsize=15)
#plt.plot(L,CT_array/L,linewidth=2,linestyle='-')
#plt.legend(fontsize=16,loc='lower right')


foo_fig = plt.gcf() # 'get current figure'
#foo_fig.savefig('general_source_CT_L.jpg', format='jpg', dpi=1000,bbox_inches = 'tight')
...
#plt.xlim([0,D_array[-1]+1])
#plt.ylim([0,70])
plt.plot(L,CT_array1/(L/sigma1),linewidth=2,linestyle='-',label='sigma=1')
plt.plot(L,CT_array2/(L/sigma2),linewidth=2,linestyle='--',label='sigma=2')
plt.plot(L,CT_array3/(L/sigma3),linewidth=2,linestyle='-.',label='sigma=3')
...
foo_fig = plt.gcf() # 'get current figure'
#foo_fig.savefig('general_source_CT_L_sigma.jpg', format='jpg', dpi=1000,bbox_inches = 'tight')
