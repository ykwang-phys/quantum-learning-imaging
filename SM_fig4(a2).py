# -*- coding: utf-8 -*-
"""
Created on Thu Jul  3 20:50:27 2025
...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 28 14:19:50 2024
...
for i in range(Q):
    ...
    r6=c5-(R1@c5)*R1-(R2@c5)*R2-(R3@c5)*R3-(R4@c5)*R4-(R5@c5)*R5
    ...
    for j in range(m):
        ...
        #error = np.linalg.norm(target_vector - np.dot(A, alpha0))/np.linalg.norm(target_vector)
        index=np.argmax(np.abs(alpha0))
        #print (alpha0,i,j)
        #print ('index',index,j)
        #print (i,j,np.abs(target_vector/A[:,j]))
        error1 = np.linalg.norm(target_vector-A[:,j])**2
        error2 = np.linalg.norm(target_vector+A[:,j])**2
        
        error_array[j,i]=np.min([error1,error2])
...
#Note the eigenvalues for direct imaging is too large and over the precision limit already for  beta3 when alpha~0.03
#We can only provide the deviation for the first three eigenvectors due to precision limit

print (1/l1[-1,:]-1)

foo_fig = plt.gcf() # 'get current figure'
foo_fig.savefig('deviation_direct_imaging.jpg', format='jpg', dpi=1000,bbox_inches = 'tight')
...
plt.plot(r[:,-1,0],linewidth=2,linestyle='-',label='r__-1')
plt.plot(r[:,-2,0],linewidth=2,linestyle='--',label='r__-2')
plt.plot(r[:,-3,0],linewidth=2,linestyle='-.',label='r__-3')
#plt.plot(r[:,-4,0],linewidth=2,linestyle='dotted',label='r____4')
...
#plt.plot(D_array,np.array([1/(M-1)]*n),label='asymptotic',linestyle=':')
plt.legend(fontsize=17, loc='upper left', ncol=1, bbox_to_anchor=(0, 1))



foo_fig = plt.gcf() # 'get current figure'
#foo_fig.savefig('eigenvector_direct_y.jpg', format='jpg', dpi=1000,bbox_inches = 'tight')
